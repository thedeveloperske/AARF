...
setFamily([]);
    ...
    postData(frmData, "fetch_member_data").then((data) => {

      if (data.length != 0) {
       setPrincipalData(data);
       setModalIsOpen(true);
      } else {
        setResponse(["Could not find member(s)"]);
  ...
  const closeModal2 = () => {
    ...
    if (!response.includes("Could not find member(s)")) {
       window.location.replace("query-by-member-name");
     }
  };
  ...
  const choosePrincipal = (e) => {
    ...
    //fetching principal data
    getOneData("fetch_one_principal_data", arr[0] + "-00").then((data) => {
      data.map((dt) => {
        setPrincipal(dt);
      });
    });
                ...
                return (
                  <tr>
                    ...
                    <td>
                      <input
                        type="text"
                        value={data.full_name_principal}
                        className="form-control"
                        readOnly
                      />
                    </td>
...
export default QueryByMemberName;
