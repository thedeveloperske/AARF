...
const QueryByMemberNumber = () => {
  ...
  const closeModal = () => {
    ...
    if (response > 1) {
      window.location.replace("query-by-member-number");
    }
  };
    ...
    getOneData(
      ...
      .then((data) => {
        ...
        if (data.member_info.length != 0) {
          ...
          //fetch member acceptance data
          data.member_acceptance.map((dt) => {
            setMemberAcceptance(dt);
          });
          //fetch member anniversary data
          ...
          setResponse(["Could not find member!"]);
          setModalIsOpen(true);
        }
      })
      .catch((error) => console.log(error));
  ...
  const submitFrm = (e) => {
    ...
    frmData.append("corp_id", corpData.corp_id);
...
export default QueryByMemberNumber;
